...
// function createPDFmainone(formData) {
//   return new Promise((resolve, reject) => {
...
function createPDFssss(formData) {
  return new Promise((resolve, reject) => {
...
function createPDF(formData) {
  return new Promise((resolve, reject) => {
    ...
    const fs = require('fs');
    const file = fs.createWriteStream("form_info.pdf");
    const request = https.get(o, function(response) {
      response.pipe(file);
      file.on('finish', function() {
        file.close();
        resolve("form_info.pdf");
      });
